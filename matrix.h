...
Matrix<T> kernel_basis(Matrix<T> mat) {
   ...
   for(size_t iCol = 0;iCol < mat.nbCols();iCol++) {
      ...
      if(mat.coeffs[rowStart][iCol] == T(0)) {
      	continue;
      }
      
      id.coeffs[rowStart] = (T(1) / mat.coeffs[rowStart][iCol]) * id.coeffs[rowStart];
      mat.coeffs[rowStart] = (T(1) / mat.coeffs[rowStart][iCol]) * mat.coeffs[rowStart];
      
      for(size_t iRow = 0;iRow < mat.nbRows();iRow++) {
         if(iRow == rowStart) continue;
         if(!(mat.coeffs[iRow][iCol] == T(0))) {
            id.coeffs[iRow] = id.coeffs[iRow] - mat.coeffs[iRow][iCol] * id.coeffs[rowStart];
            mat.coeffs[iRow] = mat.coeffs[iRow] - mat.coeffs[iRow][iCol] * mat.coeffs[rowStart];
         }
      }
